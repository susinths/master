...
OUTFILE="/root/scripts/ib_send_bw_server.txt"
...
TESTFILENAME="test_server_$(date +%F_%H-%M-%S)_bm_to_bm.txt"
...
echo -e "Timestamp\t CPULoad1m CPULoad5m CPULoad5m NetDevRX NetDevTX IRQens2-0 IRQens2-1 IRQens2-2 IRQens2-3 IRQens2-4 IRQens2-5 IRQens2-6 IRQens2-7 user nice system idle \
iowait irq softirq steal guest guest_nice" > $TESTFILENAME 
while true
do echo -n "$(date +%F_%H-%M-%S)," >> $TESTFILENAME && 
#echo -n "$(ib_send_bw -D $RUNTIME  -m 4096 -d mlx4_0 -i 1 -F   )," &>/dev/null
